...
_FC100_DATASET_DIR = '/home/cdesilva/projects/def-cdesilva/jing/datasets/FC100/'
...
class FewShotDataloader():
    ...
    def sample_test_examples_for_base_categories(self, Kbase, nTestBase):
        ...
        if len(Kbase) > 0:
            ...
            ####################### Revision #########################
            ######### delete the per class sampling here #############
            #########################################################################################################
            # per class sampling is implemented here with the randomly selected index for base and novel categories #
            # the output indices are used by create_Examples_Tensor_Data function to sample data from the dataset   #
            # we might only need to modify here to set per_class augment to False for both training and validation  #
            ##########################################################################################################
    ...
    def createExamplesTensorData(self, examples):
        ...
        indices = torch.LongTensor([img_idx for img_idx, _ in examples])
    ...
    def get_iterator(self, epoch=0):
        ...
        def load_function(iter_idx):
            ...
            Xt, Yt, IndexT = self.createExamplesTensorData(Test)
            Kall = torch.LongTensor(Kall)
            if len(Exemplars) > 0:
                Xe, Ye, IndexE = self.createExamplesTensorData(Exemplars)
                return Xe, Ye, Xt, Yt, IndexE, IndexT
            else:
                return Xt, Yt, IndexT, IndexT
